...
%inicio algoritmo
	...
	\Input{Una matriz O de $n*m$ y una matriz R de $l*k$, donde l es m'ultiplo de n y k de m.}
	\Output{R con el valor de la matriz de submuestreo  de O.}
	\BlankLine
...
%inicio algoritmo
	...
	\caption{Reducci'on por promedio}
	\Input{Una matriz O de $n*m$ y una matriz R de $l*k$, donde l es m'ultiplo de n y k de m.}
	\Output{R con el valor de la matriz de promedios de O.}
	\BlankLine
	...
	\For {i de 0 a l - 1}{
	  \For {j de 0 a k - 1}{
	    ...
	    \For {a de n*stepX a (n+1)*stepX}{
	      \For {b de m*stepX a (m+1)*stepX}{
	        R(i,j) += O(a,b)
	      }
	    }
...
%	Construyo una solucion $S$ a partir de $B$ y sustituyendo de abajo hacia arriba la matriz triangulada $A$\;
%	\Return{$S$}\;
	\end{algorithm}
	\decmargin{1em}
%fin algoritmo
